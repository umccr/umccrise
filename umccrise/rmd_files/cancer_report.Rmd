...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  sv_all <- 
    ...
    dplyr::mutate(Gene = genes %>% str_split('&') %>% map(~ .[c(1,2)] %>% .[!is.na(.)]) %>% map_chr(~ str_c(., collapse = '&')),
                  Gene = ifelse((str_split(genes, '&') %>% map_int(length)) > 2,
                                str_c(Gene, '...', sep = ', '),
                                Gene),
                  `Other affected genes` = genes %>% str_split('&') %>% map(~ .[c(-1,-2)] %>% .[!is.na(.)]) %>% map_chr(~ ifelse(length(.) > 0, str_c(., collapse = ', '), '')),
