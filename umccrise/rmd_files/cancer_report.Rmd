...
5. Prioritize variants with [simple_sv_annotation](https://github.com/vladsaveliev/simple_sv_annotation):
   ...
   * tier2:
      ...
      - bidirectional fusion (opposite stands, likely non-coding) curated by HMF
      - breakpoint in a upstream or downstream region of a cancer gene
   * tier3:
      - any other fusion
      - any other exon loss
      - LoF of a tumor suppressor
      - bidirectional fusion known from FusionCatcher
...
if (length(readLines(con = params$somatic_sv, n = 2)) > 1) {
  sv_all <- 
    ...
    dplyr::mutate(effect = ifelse(str_detect(detail, 'LOF'), 'LoF_in_TS', effect)) %>% 
    dplyr::filter(effect != "LoF_in_TS" | gene %in% tsgenes) %>% 
