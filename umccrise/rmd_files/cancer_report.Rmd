...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  sv_unmelted <-
    ...
    dplyr::mutate(
      AF_BPI = ifelse(
        is.na(AF_BPI), NA, paste0(AF_BPI, " (", AF_BPI1, ",", AF_BPI2, ")")),
      AF_PURPLE = ifelse(
        is.na(AF_PURPLE), NA, paste0(AF_PURPLE, " (", AF_PURPLE1, ", ", AF_PURPLE2, ")")),
      CN_PURPLE = ifelse(
        is.na(CN_PURPLE), NA, paste0(CN_PURPLE, " (", CN_PURPLE1, ", ", CN_PURPLE2, ")")),
      CN_change_PURPLE = ifelse(
        is.na(CN_change_PURPLE), NA, paste0(CN_change_PURPLE, " (", CN_change_PURPLE1, ", ", CN_change_PURPLE2, ")"))
    ) %>%
...
sv_summary <-
  list(raw = sv_unmelted %>% dplyr::select(svtype),
       annot = sv_all %>% dplyr::select(svtype = Type)) %>%
...
qc_summary_all <- purple_qc_summary %>%
  ...
  dplyr::bind_rows(
    dplyr::tribble(
      ...
      "SVs (raw)", paste(rev(sv_summary$raw), collapse = ", "),
      "SVs (annotated)", paste(rev(sv_summary$annot), collapse = ", "),
...
cat(c('<table><tr valign="top"><td>', sv_tier_vs_svtype()$before, '</td>', '<td>', sv_tier_vs_svtype()$after, '</td></tr></table>'), sep = '')
```

### Raw Variants {#sv-raw}

```{r svtab_raw}
svtab_raw_BND <- sv_unmelted %>%
  ...
  dplyr::mutate(BND_ID = group_indices(),
                ...
                BND_ID = ifelse(svtype == "BND", BND_ID, NA_integer_),
                BND_mate = ifelse(svtype == "BND", BND_mate, NA_character_)) %>%
  dplyr::ungroup()

svtab_raw_other <- sv_unmelted %>%
  dplyr::filter(svtype != "BND")

svtab_raw <-
  dplyr::bind_rows(svtab_raw_BND, svtab_raw_other) %>%
  dplyr::select(
    varnum, Chr = chrom, Start, End = END_BPI,
    Tier = tier, Type = svtype,
    nann, ID, BND_ID, BND_mate,
    SR_PR_alt, Ploidy, AF_PURPLE, AF_BPI,
    CNC = CN_change_PURPLE, CN = CN_PURPLE, SR_PR_ref,
    SScore = somaticscore) %>%
  dplyr::arrange(Tier, varnum)
