...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  ...
  sv_all <- sv_unmelted %>%
    dplyr::mutate(n_annot = stringr::str_count(annotation, ",") + 1,
    ...
    dplyr::mutate(n_genes = stringr::str_count(Genes, "&") + 1,
                  ...
                  chrom = ifelse(params$genome_build %in% c("hg19", "hg38"), sub("chr", "", chrom), chrom)) %>%
    tidyr::separate(Effect, c("Effect", "Other effects"), sep = '&', extra = "merge", fill = "right") %>%
    dplyr::select(
      Chr = chrom, Start = start, Start2 = START_BPI, End = end, End2 = END_BPI,
      Tier, Type = svtype, Gene, Effect, ID, n_annot, n_genes, VarNum, Detail, SR, PR, Ploidy,
    ...
    dplyr::distinct() %>%
    dplyr::arrange(Tier, Effect, desc(AF_PURPLE_avg), Gene)
} else {
