...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  sv_unmelted <-
    readr::read_tsv(sv_path, col_names = TRUE, col_types = "ccciiccccciiccccdcic") %>%
  ...
  sv_all <- sv_unmelted %>%
    ...
    dplyr::select(
      Chr = chrom, Start = start, End = end, End2 = END_BPI,
      Tier, Type = svtype, Gene, Effect, ID, Detail,
      AF_PURPLE_avg, AF_BPI_avg, CN_chg_avg = CN_change_PURPLE,
      SR, PR, CN_avg = CN_PURPLE, Ploidy, PURPLE_status, VarNum,
    ...
    dplyr::distinct() %>%
    dplyr::arrange(Tier, Effect, desc(AF_PURPLE), Gene)
} else {
...
* __Track2__: <span style="color:#8A2BE2">Beta Allele Frequency</span>.
  Given that the BAF points correspond to allele frequencies of common heterozygous SNPs,
