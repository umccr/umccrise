...
The "effect" columns could be one of the following:
   ...
   * "exon_loss" - a single or few exons where deleted, but not the whole gene
   ...
   * "LOF" - any loss of function event (according to SnpEff) in a tumor suppressor gene
   * All other types are unprioritized, but you can refer to them at [SnpEff documentation](http://snpeff.sourceforge.net/SnpEff_manual.html#input) -> Effect prediction details.

</details>

```{r sv_prioritize, message=FALSE}
col_types <- "ccciiccccciic"
sv_all = NULL

sv_path = params$somatic_sv
# sv_path = '/Users/vsaveliev/spa/tmp/umccrise_test_sv_newtsv_cluster9/E180__PRJ180261_E180-T01-D/structural/E180__PRJ180261_E180-T01-D-manta.tsv'
# sv_path = '/Users/vsaveliev/spa/tmp/umccrise_test_sv_newtsv_cluster9/E183__PRJ180263_E183_T01_D/structural/E183__PRJ180263_E183_T01_D-manta.tsv'

# d =     readr::read_tsv(sv_path, col_names = TRUE, col_types = col_types) %>%
#     dplyr::select(-caller, -sample) %>% 
#     
#     tidyr::separate(BPI_AF, c("AF_BP1", "AF_BP2"), ",") %>% 
#     dplyr::mutate(AF = (as.double(AF_BP1) + as.double(AF_BP2)) / 1,
#                   AF = AF * 100,
...
#     dplyr::mutate(PE = as.integer(PE_ALT)) %>% 
#     
...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  sv_all <- 
    ...
    dplyr::mutate(AF1 = as.double(AF1) * 100,
                  AF2 = as.double(AF2) * 100,
                  AF = (AF1 + AF2) / 2,
                  AF = format(AF, nsmall = 2),
                  AF1 = format(AF1, nsmall = 2),
                  AF2 = format(AF2, nsmall = 2)
                  ) %>%
