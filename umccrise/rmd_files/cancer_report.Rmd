...
params_local <- list(
  ...
  somatic_sv='data/SFRC01115__PRJ180680_SFRC01115T-sv-prioritize-manta-pass.tsv',
...
AFs range from 0 to 1 (0%-100%).
We discard variants that have somatic AF < 10%.
...
```{r af_plot, warning=FALSE, eval=T}
# Global AF
af_global <-
...
## Mutational signature {.tabset .tabset-fade}
...
```{r importVCF, warning=FALSE, eval=T}
somatic_snv <-
...
```{r somProfile2, warning=FALSE, out.width="80%", eval=T}
...
```{r somProfile1, warning=FALSE, out.width="60%", eval=T}
...
```{r tran_strand_bias, warning=FALSE, message=FALSE, out.width="60%", eval=T}
...
```{r rep_strand_bias, warning=FALSE, message=FALSE, out.width="60%", eval=T}
...
including mutation types and intermutation distance.
The y-axis corresponds to the distance of a mutation with the previous mutation
and is log10 transformed. Drop-downs from the plots indicate clusters or
"hotspots" of mutations.
</details>


```{r rainfall, out.width="100%", eval=T}
...
```{r purple-qc}
fn <- file.path(params$purple_qc)
tab <- readr::read_tsv(fn, col_names = c("key", "value"), col_types = "cc")
tab %>%
  dplyr::mutate(
    key = kableExtra::cell_spec(key, bold = TRUE),
    value = toupper(value),
...
tribble(~key, ~value,
  ...
  dplyr::mutate(
    key = kableExtra::cell_spec(key, bold = TRUE),
    value = kableExtra::cell_spec(value, bold = TRUE, color = dplyr::case_when(
        value == "FEMALE" ~ "purple",
        value == "MALE" ~ "blue",
        TRUE ~ "black"))) %>%
  knitr::kable(escape = FALSE) %>%
