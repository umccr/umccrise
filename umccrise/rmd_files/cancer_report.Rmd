...
AFs, CNs, CN changes and ploidy are all purity-adjusted.
...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  sv_all <- 
    ...
    tidyr::separate(BPI_AF, c("BPI_AF1", "BPI_AF2"), ",") %>% 
    dplyr::mutate(BPI_AF1 = as.double(BPI_AF1) * 100,
                  BPI_AF2 = as.double(BPI_AF2) * 100,
                  BPI_AF = (BPI_AF1 + BPI_AF2) / 2,
                  BPI_AF = format(BPI_AF, nsmall = 2),
                  BPI_AF1 = format(BPI_AF1, nsmall = 2),
                  BPI_AF2 = format(BPI_AF2, nsmall = 2)
    ) %>% 
    tidyr::separate(AF_purity_adj, c("AF1", "AF2"), ",") %>% 
    ...
    tidyr::separate(CN, c("CN1", "CN2"), ",") %>% 
    dplyr::mutate(CN1 = as.double(CN1) * 100,
                  CN2 = as.double(CN2) * 100,
                  CN = (CN1 + CN2) / 2,
                  CN = format(CN, nsmall = 2),
                  CN1 = format(CN1, nsmall = 2),
                  CN2 = format(CN2, nsmall = 2)
    ) %>% 
    tidyr::separate(CN_change, c("CN_change1", "CN_change2"), ",") %>% 
    dplyr::mutate(CN_change1 = as.double(CN_change1) * 100,
                  CN_change2 = as.double(CN_change2) * 100,
                  CN_change = (CN_change1 + CN_change2) / 2,
                  CN_change = format(CN_change, nsmall = 2),
                  CN_change1 = format(CN_change1, nsmall = 2),
                  CN_change2 = format(CN_change2, nsmall = 2)
    ) %>% 
    
    dplyr::mutate(Ploidy = as.double(Ploidy),
    ...
    dplyr::select(Location = location, 
                  ...
                  `Is recovered by PURPLE` = PURPLE_recovered,
