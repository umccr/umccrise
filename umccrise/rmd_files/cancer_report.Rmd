...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  sv_unmelted <-
    ...
    split_sv_field(AF_BPI, is_pct = TRUE) %>%
    split_sv_field(AF_PURPLE, is_pct = TRUE) %>%
  ...
  sv_all <- sv_unmelted %>%
    ...
    dplyr::select(
      ...
      SR, PR, Ploidy, Detail, AFav_PPL = AF_PURPLE, AFav_BPI = AF_BPI,
      CNCav = CN_change_PURPLE, CNav = CN_PURPLE,
      SR_ref, PR_ref, PE, PE_ref,
      SScore = somaticscore,
      AF1 = AF_PURPLE1, AF2 = AF_PURPLE2,
      CN1 = CN_PURPLE1, CN2 = CN_PURPLE2,
      CNC1 = CN_change_PURPLE1, CNC2 = CN_change_PURPLE2,
      varnum, Transcript, ngen, ntrx, nann, neff) %>%
    dplyr::distinct() %>%
    dplyr::arrange(Tier, Effect, desc(AFav_PPL), Genes)
} else {
...
sv_BND1 <- sv_BND %>%
  dplyr::select(rownum, 1:10)
sv_BND2 <- sv_BND %>%
  dplyr::select(rownum, 11:ncol(sv_BND))
...
sv_BND_purpleinf_dt <- sv_all %>%
  dplyr::filter(Type %in% c("PURPLE_inf")) %>%
  dplyr::select(Chr, Start, Ploidy, CN1, CNC1,
...
sv_noBND1 <- sv_noBND %>%
  dplyr::mutate(Genes = ifelse(ngen > 2,
                               glue::glue("n_genes: {ngen}; see Many Genes Table"),
                               Genes)) %>%
  dplyr::select(rownum, 1:9) %>%
  dplyr::select(-Effect, everything(), Effect)

sv_noBND2 <- sv_noBND %>%
  dplyr::select(rownum, 10:ncol(sv_noBND), -c(Genes_original))

sv_noBND3 <- sv_noBND %>%
  dplyr::filter(ngen > 2) %>%
  dplyr::select(rownum, Chr:Type, Effect, varnum, ngen, Genes = Genes_original)

sv_noBND1_dt <- sv_noBND1 %>%
  DT::datatable(filter = list(position = "top", clear = FALSE, plain = TRUE),
                class = "cell-border display compact",
