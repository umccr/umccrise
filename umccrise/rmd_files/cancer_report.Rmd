...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  ...
  sv_unmelted <- somatic_sv_tsv %>%
    ...
    dplyr::mutate(
      ...
      varnum = sprintf(glue::glue("%0{nchar(nrow(somatic_sv_tsv))}d"), varnum))
  ...
  sv_unmelted_bnd <- sv_unmelted %>%
    ...
    dplyr::mutate(
      ...
      BND_ID = sprintf(glue::glue("%0{nchar(nrow(sv_unmelted))}d"), BND_ID),
  ...
  sv_unmelted <-
    ...
    dplyr::select(varnum, Tier1 = tier,
                  Chr = chrom, Start, End,
                  Type = svtype,
  ...
  sv_all <- sv_unmelted %>%
    ...
    tidyr::separate(
      annotation, c('Event', 'Effect', 'Genes', 'Transcript', 'Detail', 'Tier2'),
      sep = '\\|', convert = FALSE) %>%
    dplyr::mutate(
    ...
    dplyr::distinct() %>%
    dplyr::arrange(Tier1, Genes, Effect)
} else {
...
sv_col_descr <- dplyr::tribble(
  ...
  "Tier1", "Max priority of the event (from 'simple_sv_annotation': 1 highest, 4 lowest)",
  "Tier2", "Priority of the specific event",
  "Chr", "Chromosome",
...
sv_tier_vs_svtype <- function() {
  l <- list(unmelted = sv_unmelted %>% dplyr::select(Tier1, Type),
            melted = sv_all %>% dplyr::select(Tier1, Type)) %>%
    purrr::map(function(x) addmargins(table(x, useNA = 'ifany')))

  unmelted <- knitr::kable(l$unmelted, format = "html", caption = "SV type by tier before breaking down by annotation", output = FALSE) %>%
    kableExtra::kable_styling(full_width = FALSE, position = "float_right")
  melted <- knitr::kable(l$melted, format = "html", caption = "SV type by tier after breaking down by annotation", output = FALSE) %>%
    kableExtra::kable_styling(full_width = FALSE, position = "float_right")
