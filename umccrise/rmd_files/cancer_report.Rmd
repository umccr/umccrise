...
params_spartan <- list(
  ...
  af_global=      '~/spa/tmp/umccrise_test_sv_newtsv_cluster2/work/E180__PRJ180261_E180-T01-D/rmd/afs/af_tumor.txt',
  af_keygenes=    '~/spa/tmp/umccrise_test_sv_newtsv_cluster2/work/E180__PRJ180261_E180-T01-D/rmd/afs/af_tumor_keygenes.txt',
  somatic_snv=    '~/spa/tmp/umccrise_test_sv_newtsv_cluster2/work/E180__PRJ180261_E180-T01-D/rmd/ensemble-with_chr_prefix.vcf',
  somatic_sv='/Users/vsaveliev/spa/tmp/umccrise_test_sv_newtsv_cluster2/E180__PRJ180261_E180-T01-D/structural/E180__PRJ180261_E180-T01-D-manta.tsv',
  purple_gene_cnv='~/spa/tmp/umccrise_test_sv_newtsv_cluster2/work/E180__PRJ180261_E180-T01-D/rmd/purple.tsv',
...
params_local <- list(
  ...
  tsgenes='/Users/pdiakumis/Desktop/projects/umccr/NGS_Utils/ngs_utils/reference_data/key_genes/umccr_cancer_genes.tsgenes.latest.genes',
...
The variants are called in bcbio by 3 different callers, and calls supported by at least 2 of 3 are used ("ensemble" approach).
Unless only a single caller was used due to technical reasons (e.g. difficult highly mutated FFPE sample).
...
5. Prioritize variants with [simple_sv_annotation](https://github.com/vladsaveliev/simple_sv_annotation):
   * tier1: known gene fusion (curated by [HMF](https://resources.hartwigmedicalfoundation.nl))
   * tier2:
      - exon loss in [cancer genes](https://github.com/vladsaveliev/NGS_Utils/blob/master/ngs_utils/reference_data/key_genes/umccr_cancer_genes.2019-02-12.tsv)
      - fusion known in the literature, but not a high confidence (from [FusionCatcher list](https://github.com/ndaniel/fusioncatcher/blob/master/bin/generate_known.py))
      - fusion affecting a cancer gene
      - breakpoint in a upstream or downstream region of a cancer gene
      - LoF of a tumor suppressor
   * tier3:
      - any other fusion
      - any other exon loss
...
The "effect" columns could be one of the following:
   * "gene_fusion" - could lead to a coding fusion. If also tier=1 or 2, means it's a known fusion in the literature, regardless of order.
   * "bidirectional_gene_fusion" is likely non-coding (opposing frames, _if_ inference correct). Tier=2 or 3 means a known pair, though again regardless of direction and order.
   * "EXON_DEL" - a single or few exons where deleted, but not the whole gene
   * "downstream_gene_variant" or "upstream_gene_variant" - an even down/upstream of a cancer gene, might mean a gene falling under control of a promoter of another gene
   * "LoF_TS" - any loss of function event (according to SnpEFF) in a tumor suppressor gene

</details>

```{r sv_prioritize, message=FALSE}
col_types <- "ccciiccccciicc"
sv_all = NULL

if (length(readLines(con = params$somatic_sv, n = 2)) > 1) {
  sv_all <- 
    readr::read_tsv(params$somatic_sv, col_names = TRUE, col_types = col_types) %>%
    dplyr::select(-caller, -sample) %>% 
    
    tidyr::separate(BPI_AF, c("AF1", "AF2"), ",") %>% 
    dplyr::mutate(AF = (as.double(AF1) + as.double(AF2)) / 1,
                  AF = AF * 100,
                  AF = format(AF, nsmall=2)) %>%
    ...
    dplyr::mutate(PR = as.integer(PR_ALT)) %>% 
    ...
    tidyr::separate(annotation,
                    c('event', 'effect', 'gene', 'transcript', 'detail', 'tier'),
                    sep = '\\|', convert = TRUE) %>%  # Unpack annotation columns
  
    dplyr::mutate(loc = str_c(chr, ':', start, sep = ''),
                  loc = ifelse(is_na(end), loc, str_c(loc, '-', end, sep = ''))) %>% 
    dplyr::rename(somScore = somaticscore) %>%
    dplyr::select(loc, event, tier, effect, detail, AF, SR, PR, gene) %>%
    dplyr::distinct() %>%
    # dplyr::mutate(chr = factor(chr, levels = c(1:22, "X", "Y", "MT"))) %>% 
    dplyr::arrange(tier, effect, desc(AF), gene)
...
file.path(params$purple_cnv) %>%
  readr::read_tsv(col_types = "ciididdccccccc") %>%
  ...
  knitr::kable(format.args = list(big.mark = ","), caption = "CNV Summary") %>%
  kableExtra::kable_styling(full_width = FALSE, position = "left")
```
...
* `Role`: oncogene ('onco'), tumor suppressor ('ts'), or both ('onco+ts'),
...
tsgenes <- readr::read_csv(params$tsgenes, col_names = c('symbol'))
