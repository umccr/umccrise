...
sv_summary_svtype <-
  list(before = sv_unmelted %>% dplyr::select(svtype),
       after = sv_all %>% dplyr::select(svtype = Type)) %>%
  purrr::map(function(x) addmargins(table(x, useNA = 'a'))) %>%
  do.call("rbind", .) %>%
  knitr::kable(escape = FALSE, caption = "SV_Type before/after annotation melting", output = FALSE) %>%
  kableExtra::kable_styling(full_width = TRUE)

sv_summary_tier <-
  list(before = sv_unmelted %>% dplyr::select(tier),
       after = sv_all %>% dplyr::select(tier = Tier)) %>%
  purrr::map(function(x) addmargins(table(x, useNA = 'a'))) %>%
  do.call("rbind", .) %>%
  knitr::kable(escape = FALSE, caption = "SV_Tier before/after annotation melting", output = FALSE) %>%
  kableExtra::kable_styling(full_width = TRUE)
...
qc_summary_all <- purple_qc_summary %>%
  ...
  knitr::kable(escape = FALSE, col.names = NULL, output = FALSE) %>%
  kableExtra::kable_styling(full_width = FALSE, position = "float_left")

cat(c('<table><tr><td>', '<h2>Summary</h2>',
      qc_summary_all, '</td>', '<td>', '<h2>SV Summary</h2>',
      sv_summary_svtype, sv_summary_tier, '</td></tr></table>'), sep = '')
```
...
sv_col_descr <- dplyr::tribble(
  ...
  "Chr", "Chromosome",
  "Start", "Start inferred by BPI (for PURPLE-inferred SVs we use POS)",
  "End", "End inferred by BPI (PURPLE-inferred SVs don't have an end)",
  "ID", "ID of BND from Manta (or PURPLE for PURPLE-inferred SVs))",
  "BND_ID", "ID of BND pair simplified (numbers based on Manta ID alphanumeric order)",
  ...
  "Effect", glue::glue(
    "SV effect (based on {htmltools::a(href='http://snpeff.sourceforge.net/SnpEff_manual.html#input', 'SnpEff Effect Sequence Ontology')}",
    " - abbreviations are shown in Effects table below)"),
...
```{r results='asis'}
sv_summary <- function() {
  l <- list(before = sv_unmelted %>% dplyr::select(Tier = tier, svtype),
            after = sv_all %>% dplyr::select(Tier, svtype = Type)) %>%
    purrr::map(function(x) addmargins(table(x)))

  before <- knitr::kable(l$before, format = "html", caption = "SV type by tier before breaking down by annotation", output = FALSE) %>%
    kableExtra::kable_styling(full_width = FALSE, position = "float_right")
  after <- knitr::kable(l$after, format = "html", caption = "SV type by tier after breaking down by annotation", output = FALSE) %>%
    kableExtra::kable_styling(full_width = FALSE, position = "float_right")

  list(before = before, after = after)
}

cat(c('<table><tr valign="top"><td>', sv_summary()$before, '</td>', '<td>', sv_summary()$after, '</td></tr></table>'), sep = '')
```
