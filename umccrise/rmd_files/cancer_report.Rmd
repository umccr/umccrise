...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  ...
  sv_all <- sv_unmelted %>%
    ...
    dplyr::mutate(
      ...
      Effect = Effect %>% stringr::str_replace_all('&', ', ')) %>%
    dplyr::select(
...
```{r mutationalpatterns-rainfall, out.width="90%", fig.width=14}
...
```{r svtab_BND}
sv_col_ind_js <- function(d, x) {
  ntab <- setNames((1:ncol(d) - 1), names(d))
...
sv_BND <- sv_all %>%
  ...
  dplyr::select(nrow, dplyr::everything()) # nrow is first col

sv_BND_main <- sv_BND %>%
  dplyr::select(1:13)
sv_BND_other <- sv_BND %>%
  dplyr::select(nrow, 14:ncol(sv_BND))
...
sv_BND_purpledt <- sv_all %>%
  dplyr::filter(Type %in% c("PURPLE_inf")) %>%
  dplyr::select(Chr, Start, Ploidy, CN, CNC,
                Detail, Tier, Type, Effect, ID, varnum) %>%
  DT::datatable(filter = list(position = "top", clear = FALSE, plain = TRUE),
                class = "cell-border display compact",
                rownames = FALSE, extensions = c("Scroller", "Buttons", "KeyTable"),
                options = list(
...
sv_noBND_main <- sv_noBND %>%
  ...
  dplyr::select(nrow, varnum, Type, Chr, Start, End, Tier,
                Genes, Transcript, Detail, Effect, SR_PR_alt, AF_PURPLE)

sv_noBND_other <- sv_noBND %>%
  dplyr::select(varnum, Ploidy, SR_PR_alt, AF_PURPLE, AF_BPI, CNC, CN, SR_PR_ref, SScore) %>%
  dplyr::distinct()

sv_noBND_manygenes <- sv_noBND %>%
  dplyr::filter(ngen > max_genes) %>%
  dplyr::select(nrow, varnum, Chr, Start, End, Tier, Type,
...
```{r purple-plot-copynumber, out.width="40%"}
...
```{r purple-plot-purityrange, out.width="40%"}
...
```{r purple-plot-somaticclonality, out.width="50%"}
...
```{r purple-plot-segment, out.width="40%"}
