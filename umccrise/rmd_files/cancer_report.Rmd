...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  ...
  sv_all <- sv_unmelted %>%
    dplyr::mutate(nann = stringr::str_count(annotation, ","),
                  annotation = strsplit(annotation, ',')) %>%
    tidyr::unnest(annotation) %>% # unpack multiple annotations per region
    tidyr::separate(annotation, c('Event', 'Effect', 'Genes', 'Transcript', 'Detail', 'Tier'),
                    sep = '\\|', convert = TRUE) %>%  # Unpack annotation columns
    dplyr::mutate(ngen = Genes %>% stringr::str_count("&"),
                  Genes = Genes %>% stringr::str_replace_all('&', ', '),
                  Transcript = Transcript %>% stringr::str_replace_all('&', ', '),
                  Ploidy = round(as.double(Ploidy_PURPLE), 2),
                  chrom = sub("chr", "", chrom), chrom) %>%
    tidyr::separate(Effect, c("Effect", "Other effects"), sep = '&', extra = "merge", fill = "right") %>%
    dplyr::select(
      Chr = chrom, Start = START_BPI, End = END_BPI,
      Tier, Type = svtype, Genes, Effect, ID, ngen, nann, varnum, Detail, SR, PR, Ploidy,
      AF_PURPLE_avg = AF_PURPLE, AF_BPI_avg = AF_BPI,
      CN_chg_avg = CN_change_PURPLE, CN_avg = CN_PURPLE,
      PURPLE_status, Start_old = start, End_old = end, 
      SR_ref, PR_ref, PE, PE_ref,
      SomaticScore = somaticscore,
      Transcript, `Other effects`,
      AF_bp1_PURPLE = AF_PURPLE1, AF_bp2_PURPLE = AF_PURPLE2,
      AF_bp1_BPI = AF_BPI1, AF_bp2_BPI = AF_BPI2,
      CN_bp1 = CN_PURPLE1, CN_bp2 = CN_PURPLE2,
      CN_chg_bp1 = CN_change_PURPLE1, CN_chg_bp2 = CN_change_PURPLE2) %>%
    dplyr::distinct() %>%
    dplyr::arrange(Tier, Effect, desc(AF_PURPLE_avg), Genes)
} else {
...
sv_all_col_ind_js <- function(x) {
  ntab <- setNames((1:ncol(sv_all) - 1), names(sv_all))
  sort(unname(ntab[x]))
  ...
  DT::datatable(filter = list(position = "top", clear = FALSE, plain = TRUE),
                ...
                options = list(
                  columnDefs = list(list(
                    targets = sv_all_col_ind_js(c("Genes", "Transcript", "Other effects")),
