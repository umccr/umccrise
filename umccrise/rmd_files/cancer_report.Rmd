...
Sorted in order: tier, effect AF.
...
if (length(readLines(con = params$somatic_sv, n = 2)) > 1) {
  sv_all <- 
    ...
    dplyr::mutate(AF1 = as.double(AF1)) %>% 
    dplyr::mutate(AF2 = as.double(AF2)) %>% 
    dplyr::mutate(AF = (AF1 + AF2) / 1) %>% 
    dplyr::mutate(AF = AF * 100) %>% 
    dplyr::mutate(AF = format(AF, nsmall=2)) %>%
...
* `GeneType`: oncogene ('onco'), tumor suppressor ('ts'), or both ('onco+ts'),
...
purple_gene_cnv %>%
  ...
  dplyr::select(Gene,  MinCN = MinCopyNumber, MaxCN = MaxCopyNumber, 
                ...
                ChrBand = ChromosomeBand, GeneType) %>% 
