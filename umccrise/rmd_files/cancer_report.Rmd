...
sig_probs2 <- file.path("misc/sig/sigProfiler_SBS_signatures_2019_05_22.csv")
cancer_signatures2 <-
...
purple_purity <-
  ...
  readr::read_tsv(col_types = cols(.default = col_double(),
                                   ...
                                   wholeGenomeDuplication = col_logical(),
                                   MSI = col_character()
                                   )) %>%
...
purple_qc_summary <- dplyr::tribble(
  ...
  'MSI', glue('{p["msStatus"]}'), "Microsatellite instability status",
  'MSI indels/Mb', glue('{p["msIndelsPerMb"]}'), "Microsatellite instability indels per megabase",
...
Rainfall plots visualizes the distribution of mutations along the genome, with mutation types
