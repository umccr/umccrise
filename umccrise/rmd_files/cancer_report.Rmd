...
if (length(readLines(con = sv_path, n = 2)) > 1) {
  ...
  sv_all <- sv_unmelted %>% # TODO: unnest(annotation = c(...)) solves the warning
...
```{r mutationalpatterns-rainfall, out.width="100%", fig.width=14}
...
* __Track3__: Total Copy Number Changes
* __Track4__: Minor Copy Number Changes
* __Track5__ (Inner circle): Observed Structural Variants
...
* If the sample _is not_ FFPE:
  ...
  * output (`final`): `{batch}/structural/{batch}-manta.vcf.gz`
...
dplyr::tribble(
  ...
  "BPI_AF", "INFO/BPI_AF: AF at each breakpoint",
  ...
  "AF", "INFO/PURPLE_AF: AF at each breakend (purity adjusted)",
  "CN", "INFO/PURPLE_CN: CN at each breakend (purity adjusted)",
  "CN_change", "INFO/PURPLE_CN_CHANGE: change in CN at each breakend (purity adjusted)",
...
1. Use PURPLE copy number caller to infer more SV calls from copy number transitions (marked as "From_CNV")
